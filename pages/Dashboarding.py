import streamlit as st

st.title("📊 Dashboarding and APIs", anchor=False)

st.markdown("""
Building this app has been a useful learning experience
where that allowed me to learn to use streamlit. Streamlit is often
used to create a simple user interface with flexible 
use cases these include:

- Dashboard for data

- API interaction

- Prototype machine learning models

There are lots of other use cases and streamlit is 
a very useful tool for data scientists.

This page gives me a chance to demonstrate my ability
to deliver on some valuable uses of Streamlit.
 
""")

st.divider()

st.markdown(
    """
    <h2 style="font-size:32px;">
        Dashboarding
    </h2>
    """,
    unsafe_allow_html=True,
)

st.markdown("""
    A dashboard can help businesses gain insights
    from their data in real time to help make intelligent
    decisions.

    In this example I will load data from 
    [Yahoo Finance](https://uk.finance.yahoo.com/) using their
    publicly accessible API and present some live data. This 
    methodology is easily applicable to load data from a 
    database.
""")

st.divider()


